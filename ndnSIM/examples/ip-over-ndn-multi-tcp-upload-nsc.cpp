...
#include "ip-over-ndn-multi-tcp-upload-nsc/ipoc-client.hpp"
#include "ip-over-ndn-multi-tcp-upload-nsc/gateway-app.hpp"
#include "ip-over-ndn-multi-tcp-upload-nsc/parse-config.hpp"
...
NS_LOG_COMPONENT_DEFINE("ndn.IpOverNdnMultiFileUpload");
...
main(int argc, char* argv[])
{
    ...
    // set the errorrate for the 2nd interface
    d0d1.Get (1)->SetAttribute ("ReceiveErrorModel", PointerValue (em));
    d0d1.Get (1)->TraceConnectWithoutContext ("PhyRxDrop", MakeCallback (&RxDrop));
    ...
    Ptr<Ipv4> n3Ipv4 = nodes.Get(3)->GetObject<Ipv4> (); // Get Ipv4 instance of the node
    ...
    for (const auto& app : pc.p_apps) {
        ...
        FileTransferHelper clientHelper ("ns3::TcpSocketFactory",
                                  Address (InetSocketAddress (Ipv4Address (n3Ipv4->GetAddress (1, 0).GetLocal ()), 8080)));
    ...
    ApplicationContainer cltApp = sink.Install (nodes.Get (3));
    ...
    std::string simName = "ip-over-ndn-multi-tcp-upload-nsc-" + converter.str();
