...
#include "pure-ip-multi-tcp-download-nsc/parse-config.hpp"
 ...
 *
 *  NS_LOG=ndn.IpocClient:ndn.GatewayApp ./waf --run "pure-ip-single-tcp-download"
 */
...
main(int argc, char* argv[])
{
    ...
    d1d2.Get (0)->TraceConnectWithoutContext ("PhyRxDrop", MakeCallback (&RxDrop));
    ...
    for (const auto& app : pc.p_apps) {
        ...
        FileTransferHelper clientHelper ("ns3::TcpSocketFactory",
                                  Address (InetSocketAddress (Ipv4Address (n0Ipv4->GetAddress (1, 0).GetLocal ()), 8080)));
        ...
        ApplicationContainer srvApp = clientHelper.Install (nodes.Get (4));
    ...
    std::string simName = "pure-ip-multi-tcp-download-nsc-" + converter.str();
