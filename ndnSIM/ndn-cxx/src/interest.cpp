 ...
 * 
* Modified as part of IPoC Protocol
...
Interest::wireDecode(const Block& wire)
{
  ...
  // Payload object
  m_payloadCached.reset();
  val = m_wire.find(tlv::IpPacket);
  if (val != m_wire.elements_end()) {
    m_payload = (*val);
  }
  else {
...
Interest::getPayload() const
{
  if (hasPayload()) {
    if (!m_payloadCached) {
      m_payloadCached = make_shared<Block>(m_payload);
    }
    return *m_payloadCached;
  }
  BOOST_THROW_EXCEPTION(Error("There is no encapsulated Payload object"));
}
...
void
Interest::setPayload(const Block& payload)
{
  m_payload = payload;
  if (!payload.hasWire()) {
    BOOST_THROW_EXCEPTION(Error("The given payload does not have a wire format"));
  }
  m_wire.reset();
  m_payloadCached.reset();
}
