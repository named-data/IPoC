...
public: // Payload
   ...
   /**
   * @brief Check whether the Interest contains a Payload object
   * @return True if there is a link object, otherwise false
   */
  bool
  hasPayload() const;
  ...
  /**
   * @brief Set the link object for this interest
   * @param payload The link object that will be included in this interest (in wire format)
   * @post !hasSelectedDelegation()
   */
  void
...
private:
  ...
  //mutable shared_ptr<Payload> m_payloadCached;
  mutable shared_ptr<Block> m_payloadCached;
